...
const NewsSection: React.FC<NewsSectionProps> = ({ articles }) => {
  ...
  const filtered = useMemo(
    () =>
      articles.filter((a) => {
        ...
        const tagMatch =
          activeTag === 'ALL' || (Array.isArray(a.tags) && a.tags.some((tag) => tag === activeTag));
  ...
  const hasNews = Boolean(headline);
  ...
  return (
    <section id="news" className="px-6 py-24">
      <div className="glass-panel mx-auto max-w-6xl rounded-[36px] border border-white/10 bg-[rgba(6,6,15,0.95)] p-10 text-white">
          ...
          <div className="flex flex-col gap-3 text-xs uppercase tracking-[0.35em]">
            ...
            {tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  type="button"
                  className={clsx('chip-filter', activeTag === 'ALL' && 'chip-filter-active')}
                ...
                {tags.map((tag) => (
                  <button
                    key={tag}
                    type="button"
                    className={clsx('chip-filter', activeTag === tag && 'chip-filter-active')}
        ...
        {!hasNews && (
          <div className="mt-10 rounded-3xl border border-white/10 bg-white/5 px-6 py-12 text-center text-white/70">
            <p className="text-lg">暂时没有符合筛选条件的资讯</p>
            <p className="text-sm text-white/50 mt-2">尝试切换分类或标签查看其他内容</p>
          </div>
        )}
          ...
          <div className="mt-12 grid gap-8 lg:grid-cols-[1.1fr,0.9fr]">
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8">
                ...
                {Array.isArray(headline.tags) && headline.tags.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {headline.tags.map((tag) => (
                      <span
                        key={tag}
                        className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-[11px] uppercase tracking-[0.25em] text-white/80"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                <div className="mt-6 flex itemscenter justify-between text-xs text-white/60">
                  ...
                  {Array.isArray(article.tags) && article.tags.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {article.tags.map((tag) => (
                        <span
                          key={tag}
                          className="rounded-full border border-white/10 bg-white/5 px-2 py-1 text-[10px] uppercase tracking-[0.25em] text-white/70"
                        >
                          {tag}
                        </span>
                      ))}
